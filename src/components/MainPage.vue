<template>
  <div class="w-full h-full items-center flex mt-2 max-w-full min-w-0 overflow-x-hidden">
    <div class="w-full h-full max-w-full min-w-0 flex flex-col items-center mx-10">
      <div class="w-full md:w-[1280px] h-[80px] flex max-w-full min-w-0 mt-4">
      ...
      <div class="relative w-[1280px] h-[600px] mx-10 min-w-0 max-w-full">
        <div class="w-full h-full">
          <Carousel :autoplay="5000" :wrap-around="true">
            <Slide class="h-[600px] w-full" v-for="(item, index) in castleData.availableEstates.filter(c => c.main === true)" :key="item.id">
              ...
              <div class="bg-gradient-to-l from-black w-full absolute inset-0 flex justify-start items-center">
                <h1 v-show="index === 0" class="w-10 text-white font-playfair text-7xl ml-24 drop-shadow-2xl">
                  LUXURY CASTLES
                </h1>
              </div>
              <div class="w-full absolute inset-0 flex flex-col justify-center items-end space-y-10">
                <div class="w-full h-1/2 flex flex-col justify-end items-end space-y-4">
                  <h1 class="text-white font-playfair text-4xl mr-24 drop-shadow-md">
                    {{ item.name }}
                  </h1>
                  <h2 class="text-white font-playfair text-2xl mr-24">
                    {{ item.location }}
                  </h2>
                  <h3 class="text-white font-playfair text-4xl mr-24">
                    {{ item.price }}€
                  </h3>
                </div>
                <div class="flex w-full h-1/4 justify-end items-start">
                  <button class="bg-white rounded-xl w-36 h-12 mr-24 hover:bg-[#181818] hover:text-white">
                    Learn more
                  </button>
                </div>
              </div>
      ...
      <div class="w-[1280px] h-[220px] mt-12 min-w-0 max-w-full flex">
        <Carousel class="w-full h-[160px]" :wrap-around="true"  :breakpoints="breakpoints" v-bind="settings"  :transition="500">
          <Slide class="w-[300px] h-full flex flex-col" v-for="item in castleData.availableEstates.filter(c => c.main === false)" :key="item.id">
              <img class=" w-full h-[160px] rounded-2xl" :src="item.pictures" />
